...
const { registerSchema, loginSchema, verifyOtpSchema, resetPasswordSchema } = require('../utils/validators');
...
exports.register = async (req, res, next) => {
  try {
    ...
    if (existingUser) {
      // ❗ If NOT verified → allow overwrite
      if (!existingUser.isVerified) {
        // regenerate OTP + update user
        ...
        return res.status(200).json({
          message: "Please verify OTP sent in mail."
        });
      }
...
exports.getMe = async (req, res, next) => {
  try {
    const user = await userModel.findById(req.user.id).select("username email _id")
    res.json(user);
  } catch (error) {
    next(error);
  }
